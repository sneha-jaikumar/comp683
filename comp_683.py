...
scaler.fit(result_temp)
scaled_data = scaler.transform(result_temp)
filtered_pseudo = pd.DataFrame(scaled_data, columns = result_temp.columns)
#Add age column back to df now
filtered_pseudo["sample_name"] = result_1.index.str.split('_').str[0]
filtered_pseudo["Age"] = result_1.index.str.split('_').str[1]
print(filtered_pseudo)
...
# print(pseudo_bulk_1)
filtered_pseudo.to_csv("hammond_pseudobulk.csv")
...
# data_mod['cluster_label'] = adata_merge.obs['leiden'].copy()
# data_mod['age'] = adata_merge.obs['age'].copy()
# print("initial age",data_mod['age'])
...
# print(merged_df)
# # pseudo_bulk_1 =  merged_df.drop(["Gestational age (days)"], axis=1)
# # pseudo_bulk_1.insert(0,"Age(days)",merged_df."Gestational age(days)")

# # #View Fetus Pseudobulk
# # print(pseudo_bulk_1)
# merged_df.to_csv("fetus_pseudobulk.csv")
